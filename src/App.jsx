...
<button onClick={() => setPage('home')} className="flex items-center gap-2 text-zinc-400 hover:text-emerald-600 transition-all mb-12 text-[10px] uppercase tracking-widest font-bold">
 Back to home
...
return (
        ...
        {['home', 'gallery', 'planner', 'contact'].map(link => (
          <button
            ...
            className={`text-[10px] uppercase tracking-[0.3em] font-bold transition-all ${
              activePage === link 
                ? (scrolled || activePage !== 'home' ? 'text-emerald-600' : 'text-white')
  ...
  <div className={`fixed inset-0 bg-zinc-950 z-[200] transition-all duration-700 ${isMenuOpen ? 'translate-x-0' : 'translate-x-full'}`}>
    <div className="p-8 flex justify-between items-center border-b border-white/5">
      ...
      <button onClick={() => setIsMenuOpen(false)} className="text-white p-2 border border-white/10 rounded-full"><X size={24} /></button>
    </div>
    <div className="flex flex-col p-12 gap-8 text-center mt-10">
      {['home', 'gallery', 'planner', 'contact'].map(link => (
        <button key={link} onClick={() => { setPage(link); setIsMenuOpen(false); }} className={`text-4xl font-serif italic ${activePage === link ? 'text-emerald-500' : 'text-zinc-400'}`}>
  ...
  <main>
      ...
      <>
        <section className="h-screen relative flex items-center justify-center text-center px-6">
          ...
          <div className="relative z-10 max-w-4xl">
            ...
            <h1 className="text-5xl md:text-8xl font-serif italic text-white mb-12 tracking-tight leading-none animate-in fade-in zoom-in duration-700">
              Architectural <br/> Sovereignity.
            </h1>
            <div className="flex flex-wrap justify-center gap-4">
              <button onClick={() => setPage('gallery')} className="px-10 py-5 bg-white text-zinc-900 rounded-full text-[10px] font-bold uppercase tracking-widest hover:bg-emerald-500 hover:text-white transition-all">Explore Portfolio</button>
              <button onClick={() => setPage('contact')} className="px-10 py-5 border border-white/20 text-white rounded-full text-[10px] font-bold uppercase tracking-widest hover:bg-white/10 transition-all">Private Consult</button>
            </div>
          </div>
        </section>
          ...
          <div className="max-w-[1400px] mx-auto grid grid-cols-1 lg:grid-cols-2 gap-24 items-center">
            <div className="relative">
              <div className="aspect-[4/5] rounded-[3rem] overflow-hidden shadow-2xl">
                <img src="https://images.pexels.com/photos/3183150/pexels-photo-3183150.jpeg?auto=compress&cs=tinysrgb&w=800" className="w-full h-full object-cover" alt="Office" />
              </div>
              <div className="absolute -bottom-10 -right-10 bg-white p-10 rounded-3xl shadow-xl hidden md:block max-w-xs border border-zinc-100">
    ...
    {activePage === 'gallery' && (
      <section className="pt-40 pb-32 px-6 max-w-[1400px] mx-auto min-h-screen">
        <div className="mb-20">
          ...
          {PROPERTY_DATA.map(item => (
            <div key={item.id} className="group relative cursor-pointer" onClick={() => openGalleryModal(item)}>
              <div className="aspect-[16/10] overflow-hidden rounded-[2.5rem] bg-zinc-100 mb-8 relative shadow-lg">
                ...
                <button onClick={(e) => toggleWishlist(e, item.id)} className={`absolute top-6 right-6 p-4 rounded-full backdrop-blur-md transition-all ${wishlist.includes(item.id) ? 'bg-red-50 text-white' : 'bg-white/20 text-white hover:bg-white/40'}`}>
      ...
      <section className="pt-40 pb-40 px-6 max-w-[1400px] mx-auto min-h-screen">
        <div className="mb-20">
          <span className="text-emerald-600 font-bold uppercase text-[10px] tracking-[0.4em] mb-4 block flex items-center gap-2">
            <ShieldCheck size={14}/> Exclusive Tool
          </span>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-start">
          <div className="lg:col-span-5 space-y-10 bg-zinc-50 p-10 md:p-12 rounded-[3rem] border border-zinc-100 shadow-sm">
            ...
            <div className="pt-6">
              <button onClick={() => setPage('contact')} className="w-full py-5 bg-zinc-900 text-white rounded-2xl text-[10px] font-bold uppercase tracking-widest hover:bg-emerald-600 transition-all flex items-center justify-center gap-2 shadow-lg">
                Schedule Advisory <ChevronRight size={14}/>
              </button>
            </div>
          </div>
          ...
          <div className="lg:col-span-7 h-full flex flex-col justify-center">
            <div className="relative p-12 bg-white rounded-[3rem] border border-zinc-100 shadow-2xl overflow-hidden group">
              ...
              <div className="relative z-10">
                <p className="text-[10px] uppercase font-bold tracking-widest text-emerald-500 mb-6">Projected Sovereign Value</p>
                <h3 className="text-6xl md:text-8xl font-serif italic mb-8 tracking-tighter transition-all">
                  ${Math.round(futureValue).toLocaleString()}
                </h3>
                
                <div className="flex flex-wrap gap-8 pt-12 border-t border-zinc-50">
                  <div>
                    <p className="text-[9px] uppercase font-bold text-zinc-400 mb-2">Total Appreciation</p>
                    <p className="text-2xl font-serif text-emerald-600">+${Math.round(futureValue - propertyValue).toLocaleString()}</p>
                  </div>
                  <div>
                    <p className="text-[9px] uppercase font-bold text-zinc-400 mb-2">Daily Yield (Est.)</p>
                ...
                <div className="mt-12 p-6 bg-zinc-50 rounded-2xl flex items-center gap-4 border border-zinc-100">
                  ...
                  <p className="text-xs text-zinc-400 leading-relaxed italic">Values are based on NC 2026 market projections and should be verified via a private audit.</p>
                </div>
              </div>
            </div>
          </div>
    ...
    {activePage === 'contact' && (
      <section className="pt-40 pb-40 px-6 max-w-[1400px] mx-auto min-h-screen">
         <div className="grid grid-cols-1 lg:grid-cols-2 gap-24">
            <div>
               ...
               <div className="space-y-12">
                  <div className="flex items-start gap-8">
                     <div className="w-16 h-16 rounded-3xl bg-zinc-50 flex items-center justify-center text-zinc-400 border border-zinc-100"><Phone size={24}/></div>
                     <div><p className="text-[10px] uppercase tracking-[0.2em] text-zinc-300 font-bold mb-2">Private Line</p><p className="text-2xl font-light">+1 (704) 555-VELM</p></div>
                  </div>
                  <div className="flex items-start gap-8">
                     <div className="w-16 h-16 rounded-3xl bg-zinc-50 flex items-center justify-center text-zinc-400 border border-zinc-100"><Mail size={24}/></div>
                     <div><p className="text-[10px] uppercase tracking-[0.2em] text-zinc-300 font-bold mb-2">Email Dossier</p><p className="text-2xl font-light">concierge@velmor.com</p></div>
                  </div>
                  <div className="flex items-start gap-8">
                     <div className="w-16 h-16 rounded-3xl bg-zinc-50 flex items-center justify-center text-zinc-400 border border-zinc-100"><MapPin size={24}/></div>
            ...
            <div className="bg-zinc-50 p-10 md:p-20 rounded-[4rem] border border-zinc-100 shadow-sm">
               <form className="space-y-8" onSubmit={e => e.preventDefault()}>
                  <input type="text" className="w-full bg-white py-6 px-10 rounded-2xl outline-none border border-zinc-200" placeholder="Full Name" />
                  <input type="email" className="w-full bg-white py-6 px-10 rounded-2xl outline-none border border-zinc-200" placeholder="Secure Email" />
                  <textarea className="w-full bg-white py-6 px-10 rounded-2xl outline-none border border-zinc-200 h-40" placeholder="Inquiry Details"></textarea>
                  <button className="w-full py-6 bg-zinc-900 text-white rounded-2xl text-[10px] font-bold uppercase tracking-[0.4em] hover:bg-emerald-600 transition-all shadow-lg">Engage Team</button>
               </form>
            </div>
         </div>
      </section>
  ...
  {selectedItem && (
    <div className="fixed inset-0 z-[250] flex items-center justify-center p-4">
      ...
      <div className="bg-white w-full max-w-6xl h-full max-h-[90vh] rounded-[3rem] overflow-hidden relative shadow-2xl flex flex-col md:flex-row animate-in fade-in zoom-in duration-300">
        <button onClick={() => setSelectedItem(null)} className="absolute top-8 right-8 z-[300] p-3 bg-white/20 hover:bg-white/40 rounded-full text-zinc-600"><X size={24} /></button>
          ...
          <div className="h-24 bg-white flex p-3 gap-3 overflow-x-auto border-t border-zinc-100 no-scrollbar">
            {([selectedItem.img, ...selectedItem.gallery]).map((g, i) => (
              <img 
                ...
                className={`h-full aspect-square object-cover rounded-xl cursor-pointer transition-all ${activeImgIndex === i ? 'ring-4 ring-emerald-500 scale-95 opacity-100' : 'opacity-40 hover:opacity-100'}`} 
                alt={`Thumbnail ${i}`} 
              />
            ))}
          </div>
        ...
        <div className="w-full md:w-2/5 p-8 md:p-12 overflow-y-auto flex flex-col justify-between">
          <div>
            <div className="flex justify-between items-start mb-6">
               ...
               <button onClick={(e) => toggleWishlist(e, selectedItem.id)} className={`p-4 rounded-full ${wishlist.includes(selectedItem.id) ? 'bg-red-50 text-red-500' : 'bg-zinc-50 text-zinc-300'}`}>
                  <Heart size={20} fill={wishlist.includes(selectedItem.id) ? "currentColor" : "none"} />
               </button>
            </div>
            <p className="text-zinc-400 font-light leading-relaxed mb-8 italic font-serif">"{selectedItem.desc}"</p>
            <div className="space-y-6 mb-8 py-6 border-y border-zinc-100">
               <div className="flex justify-between text-sm"><span className="text-zinc-400 font-bold uppercase text-[9px]">Location</span><span>{selectedItem.location}</span></div>
               ...
               <div className="flex justify-between text-sm"><span className="text-zinc-400 font-bold uppercase text-[9px]">Area</span><span>{selectedItem.area}</span></div>
            </div>
          </div>
          ...
          >
            {inquirySent ? <><CheckCircle size={14} /> Inquiry Logged</> : <><Mail size={14} /> Request Full Dossier</>}
          </button>
        </div>
      </div>
    </div>
  ...
  <footer className="bg-zinc-950 text-white pt-24 pb-8 px-6">
    <div className="max-w-[1400px] mx-auto">
      ...
      <div className="pt-8 border-t border-white/5 flex flex-col md:flex-row justify-between items-center gap-6">
        <p className="text-[9px] text-white/20 font-bold uppercase tracking-widest">© 2026 VELMOR PROPERTY GROUP</p>
        <div className="flex gap-8 text-[9px] font-bold uppercase tracking-widest text-white/20">
           <button onClick={() => setPage('terms')} className="hover:text-white transition-all">Terms of Service</button>
           <button onClick={() => setPage('privacy')} className="hover:text-white transition-all">Privacy & Cookies</button>
        </div>
      </div>
    </div>
  </footer>
